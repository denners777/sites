...
<section role="main" id="produtos" class='page container_24 clearfix'>
  ...
  <article>
    ...
    <?php endif; ?>
    <?php 			
			  $args = array('post_type' => 'produtos', 'cat-produto' => 'rerthy-home' );			
	...
	?>
      ...
      <figcaption class="grid_12 p">
        ...
        <div> 
        	<?php 
				$IMGS = get_bloginfo('template_url').'/timthumb.php?q=75&w=70&h=70&src='.wp_get_attachment_url(get_post_thumbnail_id($post-> ID));
				$IMG = get_bloginfo('template_url').'/timthumb.php?q=100&w=470&h=245&src='.wp_get_attachment_url(get_post_thumbnail_id($post-> ID));
			?>
          ...
          <?php foreach($images as $img){ 		  
			$IMGS = get_bloginfo('template_url').'/timthumb.php?q=75&w=70&h=70&src='.$img['location'];
